...
### Исправлено
- **Карточка в MCP Registry вела на старый образ.** Версию 0.6.0 подняли во всех
  полях `server.json`, кроме тега внутри идентификатора OCI: карточка обещала
  0.6.0, а docker-пользователь получал образ 0.5.3 со старыми именами
  инструментов. Тест это ловит, но сработал уже после того, как тег ушёл.
