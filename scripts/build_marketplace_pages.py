...
PAGES = {
    "ozon-api": {
        ...
        "methods_note": (" Отдельная оговорка про Ozon: он использует POST и для чтения "
                         "тоже, а у методов, импортированных из спецификации, класс выведен "
                         "из HTTP-глагола. Поэтому часть читающих методов посчитана здесь "
                         "как запись, и гейт относится к ним строже, чем нужно. "
                         "Ошибка безопасная, но лишняя."),
