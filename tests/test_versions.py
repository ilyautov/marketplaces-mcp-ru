...
A release ships five version-bearing files (pyproject, server.json with its
package entries, the .mcpb manifest, npm/package.json). A mismatch means the registry advertises
...
def test_server_json_matches_pyproject():
    ...
    for pkg in sj["packages"]:
        ...
        elif pkg["registryType"] == "npm":
            assert pkg["identifier"] == "marketplaces-mcp-ru"
            assert pkg["version"] == v
...
def test_npm_launcher_matches_pyproject():
    """The npm launcher pins the PyPI version it runs to its own version, and the
    registry verifies npm ownership through package.json's mcpName."""
    v = _pyproject_version()
    sj = json.loads((ROOT / "server.json").read_text(encoding="utf-8"))
    pj = json.loads((ROOT / "npm" / "package.json").read_text(encoding="utf-8"))
    assert pj["version"] == v
    assert pj["mcpName"] == sj["name"]
    assert pj["bin"] == {"marketplaces-mcp-ru": "bin/marketplaces-mcp-ru.js"}
    assert (ROOT / "npm" / "bin" / "marketplaces-mcp-ru.js").exists()
